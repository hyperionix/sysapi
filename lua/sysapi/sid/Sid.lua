...
function Sid:getDomainAndUsername()
  local nameSize = ffi.new("DWORD[1]", 256)
  local domainSize = ffi.new("DWORD[1]", 256)
  ...
  local err = advapi32.LookupAccountSidA(nil, self.sid, name, nameSize, domain, domainSize, sidNameUse)
  if err ~= 0 then
